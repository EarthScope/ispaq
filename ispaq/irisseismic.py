...
_R_as_integer = ro.r(
    "base::as.integer"
)  # conversion of python integers to R integer vectors
...
_R_taupTraveltime = ro.r("IRISSeismic::taupTraveltime")
    ...
    if x is None:
        ...
        if isinstance(x, list):
            raise NotImplementedError(
                "Argument 'x' does not yet support lists of 'UTCDateTime's."
            )
        else:
            x = x.isoformat()
        ...
        raise TypeError(
            "Argument 'x' must be of type 'obspy.core.utcdatetime.UTCDateTime'."
        )

    # Pass the array of strings to the R as.integer() function
    with localconverter(ro.default_converter):
        posixct = _R_as_POSIXct(x, format="%Y-%m-%dT%H:%M:%OS", tz="GMT")
    return posixct
...
def R_Trace(
    ...
    r_trace = _R_initialize(
        r_trace,
        id=".".join([trace.id, trace.stats.mseed.dataquality]),
        stats=R_TraceHeader(
            trace.stats, latitude, longitude, elevation, depth, azimuth, dip
        ),
        Sensor=sensor,
...
def getEvent(
    client_url="https://earthquake.usgs.gov",
    starttime=None,
    endtime=None,
    minmag=None,
    maxmag=None,
    ...
    # Call the function and return a pandas dataframe with the results
    r_df = _R_getEvent(
        r_client, starttime, endtime, minmag, maxmag, magtype, mindepth, maxdepth
    )
    with localconverter(ro.default_converter + pandas2ri.converter):
        df = ro.conversion.rpy2py(r_df)
...
def getTraveltime(latitude, longitude, depth, staLatitude, staLongitude):
    ...
    # Call the function and return a pandas dataframe with the results
    r_df = _R_getTraveltime(
        r_client, latitude, longitude, depth, staLatitude, staLongitude
    )
    with localconverter(ro.default_converter + pandas2ri.converter):
        df = ro.conversion.rpy2py(r_df)
    return df

def taupTraveltime(latitude, longitude, depth, staLatitude, staLongitude):
    """
    Returns a pandas dataframe with seismic traveltime data from the
    TauP Toolkit (Crotwell, et al., 1999) as used by the IRISSeismic package
    on CRAN. 
    :param latitude: Latitude of seismic event.
    :param longitude: Longitude of seismic event.
    :param depth: depth of the seismic event.
    :param staLatitude: Latitude of seismic station.
    :param staLongitude: Longitude of seismic station.
    """
    # Call the function and return a pandas dataframe with the results
    r_df = _R_taupTraveltime(
        latitude, longitude, depth, staLatitude, staLongitude
    )
    with localconverter(ro.default_converter + pandas2ri.converter):
        df = ro.conversion.rpy2py(r_df)
    return df
...
def trim_taper_filter(stN, stE, stZ, max_length, taper, filterArgs):
    ...
    ro.r(
        "N <- IRISSeismic::butterworth(N,%s,%s,%s)"
        % (filterArgs[0], filterArgs[1], filterArgs[2])
    )
    ro.r(
        "E <- IRISSeismic::butterworth(E,%s,%s,%s)"
        % (filterArgs[0], filterArgs[1], filterArgs[2])
    )
    ro.r(
        "Z <- IRISSeismic::butterworth(Z,%s,%s,%s)"
        % (filterArgs[0], filterArgs[1], filterArgs[2])
    )
