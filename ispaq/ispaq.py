...
def main():
    ...
    try:
        ...
        importlib.util.find_spec("obspy")
    ...
    parser = argparse.ArgumentParser(
        description=" ".join(["ISPAQ version", __version__]),
        epilog=epilog_text,
        formatter_class=lambda prog: argparse.RawTextHelpFormatter(
            prog, max_help_position=35, width=82
        ),
    )
    ...
    prefs = parser.add_argument_group(
    ...
    prefs.add_argument(
        "--event_url", required=False, help="FDSN webservice or path to QuakeML file"
    )
    prefs.add_argument(
        "--resp_dir", required=False, help="path to directory with RESP files"
    )
    prefs.add_argument(
        "--output",
        required=False,
    ...
    prefs.add_argument(
        "--db_name", required=False, help="name of sqlite database file, if output=csv"
    )
    prefs.add_argument(
        "--csv_dir",
        required=False,
    ...
    prefs.add_argument(
        "--pdf_dir", required=False, help="directory to write generated PDF files"
    )
    prefs.add_argument(
        "--pdf_type",
        required=False,
    ...
    prefs.add_argument(
        ...
        action="store_true",
        default=False,
        help="if set, ISPAQ will look for local data files with Seiscomp SDS naming format \nNET.STA.LOC.CHAN.TYPE.YEAR.DAY where TYPE=D",
    )
            ...
            for metric_name in default_function["metrics"]:
                if metric_name not in ["pdf_plot", "pdf_text"]:
                    if bLogic not in ispaq_dict:
                        metricList.append(
                            metric_name
                            + "  *metric will not run with this version of ISPAQ*"
                        )
                    else:
        ...
        try:
            ...
            if df is None:
                ...
                try:
                    ...
                    utils.write_simple_df(
                        df, filepath, concierge, sigfigs=concierge.sigfigs
                    )
                except Exception as e:
                    logger.debug(e)
            ...
            if df is None:
                ...
                try:
                    ...
                    if concierge.output == "csv":
                        ...
                        logger.info(
                            "Writing sampleRate metrics to %s" % concierge.db_name
                        )
                    utils.write_simple_df(
                        df, filepath, concierge, sigfigs=concierge.sigfigs
                    )
            ...
            if df is None:
                ...
                try:
                    ...
                    if concierge.output == "csv":
                        ...
                        logger.info(
                            "Writing crossTalk metrics to %s" % concierge.db_name
                        )
                    utils.write_simple_df(
                        df, filepath, concierge, sigfigs=concierge.sigfigs
                    )
    ...
    if "pressureCorrelation" in concierge.logic_types:
        ...
        try:
            ...
            if df is None:
                ...
                try:
                    filepath = (
                        concierge.output_file_base + "_pressureCorrelationMetrics.csv"
                    )
                    if concierge.output == "csv":
                        logger.info(
                            "Writing pressureCorrelation metrics to %s" % filepath
                        )
                    elif concierge.output == "db":
                        logger.info(
                            "Writing pressureCorrelation metrics to %s"
                            % concierge.db_name
                        )
                    utils.write_simple_df(
                        df, filepath, concierge, sigfigs=concierge.sigfigs
    ...
    if "crossCorrelation" in concierge.logic_types:
        ...
        try:
            ...
            if df is None:
                ...
                try:
                    filepath = (
                        concierge.output_file_base + "_crossCorrelationMetrics.csv"
                    )
                    if concierge.output == "csv":
                        logger.info("Writing crossCorrelation metrics to %s" % filepath)
                    elif concierge.output == "db":
                        logger.info(
                            "Writing crossCorrelation metrics to %s" % concierge.db_name
                        )
                    utils.write_simple_df(
                        df, filepath, concierge, sigfigs=concierge.sigfigs
                    )
    ...
    if "orientationCheck" in concierge.logic_types:
        ...
        try:
            ...
            if df is None:
                ...
                try:
                    filepath = (
                        concierge.output_file_base + "_orientationCheckMetrics.csv"
                    )
                    if concierge.output == "csv":
                        logger.info("Writing orientationCheck metrics to %s" % filepath)
                    elif concierge.output == "db":
                        logger.info(
                            "Writing orientationCheck metrics to %s" % concierge.db_name
                        )
                    utils.write_simple_df(
                        df, filepath, concierge, sigfigs=concierge.sigfigs
                    )
            ...
            if df is None:
                ...
                try:
                    ...
                    if concierge.output == "csv":
                        ...
                        logger.info(
                            "Writing transferFunction metrics to %s" % concierge.db_name
                        )
                    utils.write_simple_df(
                        df, filepath, concierge, sigfigs=concierge.sigfigs
                    )
