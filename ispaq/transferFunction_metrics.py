...
def transferFunction_metrics(concierge):
    ...
    if (
        ...
        try:
            ...
            logger.error(
                "Could not connect to 'https:/service.earthscope.org/irisws/evalresp/1'"
            )
            return None
    ...
    if concierge.station_client is None:
        try:
            initialAvailability = concierge.get_availability(
                "transfer_function", starttime=start, endtime=end
            )
    ...
    for day in range(nday):
        ...
        try:
            availability = concierge.get_availability(
                "transfer_function", starttime=windowStart, endtime=windowEnd
            )
        ...
        networkStationPairs = (
            networkStationPairs.drop_duplicates().sort_values().reset_index(drop=True)
        )

        logger.info(
            "Calculating transferFunction metrics for %d stations for %s - %s"
            % (
                len(networkStationPairs),
                str(windowStart).split(".")[0],
                str(windowEnd).split(".")[0],
            )
        )
        ...
        for networkStation in networkStationPairs:
            ...
            stationAvailability = availability[
                (availability.network == network) & (availability.station == station)
            ...
            if stationAvailability.shape[0] == 0:
                logger.info(
                    "Skipping %s.%s,  network-station pair %s.%s has data, but no metadata."
                    % (network, station)
                )
                continue
            else:
                logger.debug(
                    "Network-Station pair %s.%s has %d channels"
                    % (network, station, stationAvailability.shape[0])
                )
            ...
            for dip in dips:
                ...
                channelAvailability = stationAvailability[
                    abs(stationAvailability.dip) == dip
                ].reset_index(drop=True)
                ...
                if dip == 90:
                    ...
                    if len(channelAvailability.location.unique()) == 1:
                        logger.info(
                            "Skipping %s because there are no other channels for comparison"
                            % (channelAvailability.snclId[0])
                        )
                        continue
                    ...
                    for combo in itertools.combinations(
                        range(channelAvailability.shape[0]), 2
                    ):
                    ...
                    for i in range(rowMatrix.shape[0]):
                        ...
                        if Zav1.location == "":
                            Zav1, Zav2 = (
                                Zav2,
                                Zav1,
                            )  # swap order to be consistent with MUSTANG output
                        ...
                        if (Zav1.location == Zav2.location) and (
                            Zav1.channel[-2:] == Zav2.channel[-2:]
                        ):
                            continue

                        logger.info(
                            "%03d Calculating transferFunction metrics for %s:%s"
                            % (idx, Zav2.snclId, Zav1.snclId)
                        )
                        idx += 1

                        # Get primary (1) and secondary (2) traces
                        try:
                            Zst1 = concierge.get_dataselect(
                                Zav1.network,
                                Zav1.station,
                                Zav1.location,
                                Zav1.channel,
                            ...
                            if str(e).lower().find("no data") > -1:
                                ...
                                logger.warning(
                                    "No data available for %s from %s: %s"
                                    % (Zav1.snclId, concierge.dataselect_url, e)
                                )
                            continue

                        try:
                            Zst2 = concierge.get_dataselect(
                                Zav2.network,
                                Zav2.station,
                                Zav2.location,
                                Zav2.channel,
                            ...
                            if str(e).lower().find("no data") > -1:
                                ...
                                logger.warning(
                                    "No data available for %s from %s: %s"
                                    % (Zav2.snclId, concierge.dataselect_url, e)
                                )
                            continue

                        sampling_rate = min(
                            utils.get_slot(Zst1, "sampling_rate"),
                            utils.get_slot(Zst2, "sampling_rate"),
                        )

                        # Get primary (1), secondary (2) and orthogonal secondary spectra
                        try:
                            Zevalresp1 = utils.getSpectra(
                                Zst1, sampling_rate, "transferFunction", concierge
                            )
                            Zevalresp2 = utils.getSpectra(
                                Zst2, sampling_rate, "transferFunction", concierge
                            )
                        except Exception as e:
                            logger.warning(
                                '"transferFunction_metrics" getSpectra failed for %s:%s: %s'
                                % (Zav1.snclId, Zav2.snclId, e)
                            )
                            continue

                        # Run the transferFunction metric ----------------------------------------
                        try:
                            df = irismustangmetrics.apply_transferFunction_metric(
                                Zst1, Zst2, Zevalresp1, Zevalresp2
                            )
                            dataframes.append(df)
                        except Exception as e:
                            logger.warning(
                                '"transfer_function" metric calculation failed for %s:%s: %s'
                                % (Zav1.snclId, Zav2.snclId, e)
                            )
                    ...
                    channelAvailability["snclPrefix"] = channelAvailability.snclId.str[
                        :-1
                    ]
                    snclPrefixes = (
                        channelAvailability.snclPrefix.drop_duplicates()
                        .sort_values()
                        .reset_index(drop=True)
                    )

                    # Make sure we've sorted by snclPrefix before we loop through and assign X or Y to each entry
                    channelAvailability = channelAvailability.sort_values(
                        by="snclPrefix"
                    )
                    ...
                    for snclPrefix in snclPrefixes:
                        xyAvailability = channelAvailability[
                            channelAvailability.snclId.str.contains(snclPrefix)
                        ]
                        ...
                        if xyAvailability.shape[0] == 2:
                            ...
                            if (diffAzim >= -93 and diffAzim <= -87) or (
                                diffAzim >= 267 and diffAzim <= 273
                            ):
                                axisList.append("Y")
                                axisList.append("X")
                            elif (diffAzim >= -273 and diffAzim <= -267) or (
                                diffAzim >= 87 and diffAzim <= 93
                            ):
                                axisList.append("X")
                                axisList.append("Y")
                            else:
                    ...
                    channelAvailability = channelAvailability[
                        channelAvailability.cartAxis != "D"
                    ].reset_index(drop=True)

                    # Separate channel availability data frame into X and Y data frames
                    # Pair them across location codes
                    XchannelAvailability = channelAvailability[
                        channelAvailability.cartAxis != "Y"
                    ].reset_index(drop=True)
                    YchannelAvailability = channelAvailability[
                        channelAvailability.cartAxis != "X"
                    ].reset_index(drop=True)

                    XrowMatrix = []
                    for combo in itertools.combinations(
                        range(XchannelAvailability.shape[0]), 2
                    ):
                        XrowMatrix.append(combo)

                    # Convert to a pandas dataframe
                    XrowMatrix = pd.DataFrame(
                        XrowMatrix, columns=["Primary", "Secondary"]
                    )

                    YrowMatrix = []
                    for combo in itertools.combinations(
                        range(YchannelAvailability.shape[0]), 2
                    ):
                        YrowMatrix.append(combo)

                    # Convert to a numpy matrix for total agreement with original R code
                    YrowMatrix = pd.DataFrame(
                        YrowMatrix, columns=["Primary", "Secondary"]
                    )
                    ...
                    for i in range(XrowMatrix.shape[0]):
                        azDiff = (
                            XchannelAvailability.azimuth[XrowMatrix.iloc[i, 0]]
                            - XchannelAvailability.azimuth[XrowMatrix.iloc[i, 1]]
                        )
                    ...
                    for i in range(YrowMatrix.shape[0]):
                        azDiff = (
                            YchannelAvailability.azimuth[YrowMatrix.iloc[i, 0]]
                            - YchannelAvailability.azimuth[YrowMatrix.iloc[i, 1]]
                        )
                    ...
                    YrowMatrixRot = YrowMatrix[YrowMatrix.azDiff != 0].reset_index(
                        drop=True
                    )
                    YrowMatrixNoRot = YrowMatrix[YrowMatrix.azDiff == 0].reset_index(
                        drop=True
                    )
                    XrowMatrixRot = XrowMatrix[XrowMatrix.azDiff != 0].reset_index(
                        drop=True
                    )
                    XrowMatrixNoRot = XrowMatrix[XrowMatrix.azDiff == 0].reset_index(
                        drop=True
                    )
                    ...
                    for i in range(2):
                        if matrixListNotToRotate[i].shape[0] != 0:
                            for j in range(matrixListNotToRotate[i].shape[0]):
                                av1 = availabilityList[i].iloc[
                                    int(matrixListNotToRotate[i].iloc[j, 0]),
                                ]
                                av2 = availabilityList[i].iloc[
                                    int(matrixListNotToRotate[i].iloc[j, 1]),
                                ]
                                ...
                                # We'll define the same instrument as one where the location code
                                # and last 2 characters of the channel code match.
                                if (av1.location == av2.location) and (
                                    av1.channel[-2:] == av2.channel[-2:]
                                ):
                                    continue

                                logger.info(
                                    "%03d Calculating transferFunction metrics for %s:%s"
                                    % (idx, av2.snclId, av1.snclId)
                                )
                                idx += 1

                                # Get primary (1) and secondary (2) traces
                                try:
                                    st1 = concierge.get_dataselect(
                                        av1.network,
                                        av1.station,
                                        av1.location,
                                        av1.channel,
                                    ...
                                    if str(e).lower().find("no data") > -1:
                                        logger.info(
                                            "No data available for %s" % (av1.snclId)
                                        )
                                    elif str(e).lower().find("multiple epochs") > -1:
                                        logger.info(
                                            "Skipping %s because multiple metadata epochs found"
                                        ...
                                        logger.warning(
                                            "No data available for %s from %s: %s"
                                            % (av1.snclId, concierge.dataselect_url, e)
                                        )
                                    continue

                                try:
                                    st2 = concierge.get_dataselect(
                                        av2.network,
                                        av2.station,
                                        av2.location,
                                        av2.channel,
                                    ...
                                    if str(e).lower().find("no data") > -1:
                                        logger.info(
                                            "No data available for %s" % (av2.snclId)
                                        )
                                    elif str(e).lower().find("multiple epochs") > -1:
                                        logger.info(
                                            "Skipping %s because multiple metadata epochs found"
                                        ...
                                        logger.warning(
                                            "No data available for %s from %s: %s"
                                            % (av2.snclId, concierge.dataselect_url, e)
                                        )
                                    continue

                                sampling_rate = min(
                                    utils.get_slot(st1, "sampling_rate"),
                                    utils.get_slot(st2, "sampling_rate"),
                                )

                                # Get primary (1), secondary (2) and orthogonal secondary spectra
                                try:
                                    evalresp1 = utils.getSpectra(
                                        st1,
                                        sampling_rate,
                                        "transferFunction",
                                        concierge,
                                    )
                                    evalresp2 = utils.getSpectra(
                                        st2,
                                        sampling_rate,
                                        "transferFunction",
                                        concierge,
                    ...
                    for i in range(2):
                        if matrixListToRotate[i].shape[0] != 0:
                            for j in range(matrixListToRotate[i].shape[0]):
                                av1 = availabilityList[i].iloc[
                                    int(matrixListToRotate[i].iloc[j, 0]),
                                ]  # Primary trace for Y (i=1) or X (i=2)
                                av2 = availabilityList[i].iloc[
                                    int(matrixListToRotate[i].iloc[j, 1]),
                                ]  # Secondary trace for Y (i=1) or X (i=2)
                                ...
                                if i == 0:
                                    av3 = availabilityList[i + 1][
                                        availabilityList[i + 1].snclPrefix
                                        == av2.snclPrefix
                                    ]
                                else:
                                    av3 = availabilityList[i - 1][
                                ...
                                try:
                                    ...
                                    if str(e).lower().find("no data") > -1:
                                        logger.info(
                                            "No data available for %s" % (av3.snclId)
                                        )
                                    elif str(e).lower().find("multiple epochs") > -1:
                                        logger.info(
                                            "Skipping %s because multiple metadata epochs found"
                                        ...
                                        logger.warning(
                                            "No data available for %s from %s: %s"
                                            % (av3.snclId, concierge.dataselect_url, e)
                                        )
                                    continue
                                ...
                                try:
                                    ...
                                    # RrotAngle = pandas2ri.py2ri(rotAngle)
                                    traceRotList = irisseismic.rotate2D(
                                        Yst2, Xst2, rotAngle
                                    )
                                except Exception as e:
                                    logger.warning(
                                        '"transferFunction_metrics Trace rotation failed for %s:%s: %s'
                                        % (av1.snclId, av2.snclId, e)
                                    )
                                    continue
                                if not isinstance(traceRotList, list):
                                    ...
                                    logger.warning(
                                        '"transferFunction_metrics Trace rotation failed for %s:%s: %s'
                                        % (av1.snclId, av2.snclId, traceRotList)
                                    )
                                    continue
                                ...
                                if av1.cartAxis == "Y":
                                    try:
                                        df = irismustangmetrics.apply_transferFunction_metric(
                                            st1, RYst2, evalresp1, RYevalresp2
                                        )
                                    except Exception as e:
                                        logger.warning(
                                            '"transfer_function" metric calculation failed for %s:%s: %s'
                                    ...
                                    try:
                                        df = irismustangmetrics.apply_transferFunction_metric(
                                            st1, RXst2, evalresp1, RXevalresp2
                                        )
                                    except Exception as e:
                                        logger.warning(
                                            '"transfer_function" metric calculation failed for %s:%s: %s'
                        ...
                        logger.info(
                            "Skipping %s -- dip %f  requires 3D rotation"
                            % (
                                channelAvailability.snclId.iloc[i],
                                channelAvailability.dip.iloc[i],
                            )
                        )
