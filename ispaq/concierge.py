...
class Concierge(object):
    ...
    def __init__(self, user_request=None, logger=None):
        ...
        if os.path.isdir(user_request.csv_dir):
            ...
            self.logger.warning(
                "csv_dir %s does not exist, creating directory" % user_request.csv_dir
            )
            try:
                ...
                self.logger.warning(
                    "Cannot create csv_dir %s, defaulting to current directory"
                    % user_request.csv_dir
                )
                self.csv_dir = "."
        ...
        if os.path.isdir(user_request.psd_dir):
            ...
            self.logger.warning(
                "psd_dir %s does not exist, creating directory" % user_request.psd_dir
            )
            try:
                ...
                self.logger.warning(
                    "Cannot create psd_dir %s, defaulting to current directory"
                    % user_request.psd_dir
                )
                self.psd_dir = "."
        ...
        if os.path.isdir(user_request.pdf_dir):
            ...
            self.logger.warning(
                "pdf_dir %s does not exist, creating directory" % user_request.pdf_dir
            )
            try:
                ...
                self.logger.warning(
                    "Cannot create pdf_dir %s, defaulting to current directory"
                    % user_request.pdf_dir
                )
                self.pdf_dir = "."
        ...
        if user_request.event_url is not None:
            if (
                user_request.event_url == "IRIS"
                or user_request.event_url.upper() == "EARTHSCOPE"
            ):
        ...
        if user_request.dataselect_url in URL_MAPPINGS.keys():
            ...
            "http://" in user_request.dataselect_url
            or "https://" in user_request.dataselect_url
        ):
            ...
            if user_request.station_url is not None:
                if user_request.station_url != user_request.dataselect_url:
                    self.logger.warning(
                        "Station_url should be the same as dataselect_url when retrieving data from FDSN webservices. Station_url '%s' does not match dataselect_url '%s'"
            ...
            if os.path.exists(os.path.abspath(user_request.dataselect_url)):
                ...
                err_msg = (
                    "Cannot find dataselect_url: '%s'" % user_request.dataselect_url
                )
                self.logger.critical(err_msg)
                raise SystemExit
        ...
        if user_request.station_url is None:
            if "http://" in self.dataselect_url or "https://" in self.dataselect_url:
                ...
                try:
                    ...
                    self.logger.info(
                        "Metrics that require metadata information cannot be calculated"
                    )
            ...
            "http://" in user_request.station_url
            or "https://" in user_request.station_url
        ):
            ...
            if os.path.exists(os.path.abspath(user_request.station_url)):
                ...
                self.logger.warning(
                    "Cannot find station_url '%s'" % user_request.station_url
                )
                self.logger.info(
                    "Metrics that require metadata information cannot be calculated"
                )
        ...
        if user_request.event_url is None:
            if any(
                map(lambda x: x in self.metric_names, event_metrics)
            ):  # only warn if calculating event metrics
                self.logger.warning("event_url is None or not specified")
                self.logger.info(
                    "Metrics that require event information cannot be calculated"
                )
            ...
            "http://" in user_request.event_url or "https://" in user_request.event_url
        ):
            ...
            if os.path.exists(os.path.abspath(user_request.event_url)):
                ...
                if any(
                    ...
                    self.logger.warning(
                        "Cannot find event_url '%s'" % user_request.event_url
                    )
                    self.logger.warning(
                        "Metrics that require event information cannot be calculated"
                    )
        ...
        if self.requested_starttime is None and self.dataselect_client is None:
            if self.requested_endtime is None:
                self.logger.info(
                    "No start or end time requested. Start and end time will be determined from local data file extents"
                )
            else:
                self.logger.info(
                    "No start time requested. Start time will be determined from local data file extents"
                )
            self.fileDates = []
            for sncl_pattern in self.sncl_patterns:
                ...
                if self.sds_files:
                    fpattern1 = "%s" % (
                        sncl_pattern + ".D" + ".[12][0-9][0-9][0-9].[0-9][0-9][0-9]"
                    )  # seiscomp sds file naming, waveform type D
                else:
                    fpattern1 = "%s" % (
                        sncl_pattern + ".[12][0-9][0-9][0-9].[0-9][0-9][0-9]"
                    )
                fpattern2 = "%s" % (fpattern1 + ".[A-Z]")
                for root, dirnames, fnames in os.walk(self.dataselect_url):
                    for fname in fnmatch.filter(fnames, fpattern1) + fnmatch.filter(
                        fnames, fpattern2
                    ):
                ...
                if len(matching_files) == 0:
                    ...
                    for _file in matching_files:
                        try:
                            ...
                            self.logger.debug(e)
                            self.logger.debug(
                                "Can't extract date from %s, %s" % (_file, e)
                            )
                            continue
            if len(self.fileDates) == 0:
                self.logger.critical(
                    "No start date could be determined. No files found"
                )
                raise SystemExit
            else:
                self.requested_starttime = min(self.fileDates)[0]
                if self.requested_endtime is None:
                    self.requested_endtime = (
                        max(self.fileDates)[0] + 86400
                    )  # add one day
                self.logger.info(
                    "Start time %s"
                    % self.requested_starttime.strftime("%Y-%m-%dT%H:%M:%S")
                )
                self.logger.info(
                    "End time %s" % self.requested_endtime.strftime("%Y-%m-%dT%H:%M:%S")
                )
        elif self.requested_starttime is None:
            self.logger.critical(
                "--starttime must be specified for dataselect_url %s" % self.station_url
            )
            raise SystemExit
        ...
        if user_request.resp_dir is None:  # use EarthScope evalresp web service
            ...
            user_request.resp_dir in URL_MAPPINGS.keys()
        ):  # use EarthScope evalresp web service
            self.resp_dir = None
        else:
            if os.path.exists(os.path.abspath(user_request.resp_dir)):
                self.resp_dir = os.path.abspath(
                    user_request.resp_dir
                )  # directory where RESP files are located
        ...
        self.logger.debug(
            ...
            self.requested_starttime.strftime("%Y-%m-%dT%H:%M:%S"),
            self.requested_endtime.strftime("%Y-%m-%dT%H:%M:%S"),
        )
    ...
    def get_availability(
            ...
            if self.initial_availability is None:
                try:
                    ...
                    if self.station_url is not None:
                        self.logger.info(
                            "Reading StationXML file %s" % self.station_url
                        )
                        sncl_inventory = obspy.read_inventory(
                            self.station_url, format="STATIONXML"
                        )

                except Exception as e:
                    err_msg = (
                        "The StationXML file: '%s' is not valid" % self.station_url
                    )
                    self.logger.debug(e)
                    self.logger.error(err_msg)
                    raise ValueError
                ...
                if "sncl_inventory" in locals():
                    for n in sncl_inventory.networks:
                        for s in n.stations:
                            for c in s.channels:
                                if (c.start_date < _endtime) and (
                                    (c.end_date > _starttime) or (c.end_date is None)
                                ):
                                    if c.end_date is None:
                                        tmpend = UTCDateTime("2599-12-31T23:59:59")
                                    else:
                                        tmpend = c.end_date
                                    snclId = self.get_sncl_pattern(
                ...
                for sncl_pattern in self.sncl_patterns:
                    ...
                    _sncl_pattern = self.get_sncl_pattern(
                        _network, _station, _location, _channel
                    )
                    self.logger.debug(
                        "Adding %s to availability dataframe" % _sncl_pattern
                    )
                    ...
                    if self.station_client is None:  # Local metadata
                        if self.dataselect_client is None:  # Local data
                            ...
                            if (
                                len(sncl_pattern.split(".")) > 4
                            ):  # expected to be quality code
                                ...
                                if self.sds_files:
                                    ...
                                    )  # SDS file naming structure, D=waveform data
                                else:
                                    fpattern1 = "%s" % (
                                        tmp_sncl_pattern
                                        + ".[12][0-9][0-9][0-9].[0-9][0-9][0-9]"
                            ...
                            for root, dirnames, fnames in os.walk(self.dataselect_url):
                                for fname in fnmatch.filter(
                                    fnames, fpattern1
                                ) + fnmatch.filter(fnames, fpattern2):
                                    if self.sds_files:
                                        position5 = fname.split(".")[5]
                                        position6 = fname.split(".")[6]
                                        if fnmatch.fnmatch(
                                            position5, "[12][0-9][0-9][0-9]"
                                        ) and fnmatch.fnmatch(
                                            position6, "[0-9][0-9][0-9]"
                                        ):
                                        ...
                                        position5 = fname.split(".")[5]
                                        if fnmatch.fnmatch(
                                            position4, "[12][0-9][0-9][0-9]"
                                        ) and fnmatch.fnmatch(
                                            position5, "[0-9][0-9][0-9]"
                                        ):
                                            ...
                                            continue
                                    file_date = (
                                        datetime.datetime(file_year, 1, 1)
                                        + datetime.timedelta(file_day - 1)
                                    ).date()
                                    ...
                                    if (
                                        file_date >= _starttime.date
                                        and file_date < _endtime
                                    ):
                                        matching_files.append(os.path.join(root, fname))
                            ...
                            if len(matching_files) == 0:
                                ...
                                for _file in matching_files:
                                    ...
                                    if not any(df.snclId.str.contains(snclId)):
                                        ...
                                        df.loc[len(df)] = [
                                            fileSNCL.split(".")[self.netOrder],
                                            fileSNCL.split(".")[self.staOrder],
                                            fileSNCL.split(".")[self.locOrder],
                                            fileSNCL.split(".")[self.chanOrder],
                                            None,
                                            None,
                                            None,
                                            None,
        ...
        for sncl_pattern in self.sncl_patterns:
            ...
            if self.station_client is None:
                ...
                self.logger.debug(
                    "read IRISPH5 station web services %s/%s for %s,%s,%s,%s,%s,%s"
                    % (
                        self.station_url,
                        self.station_type,
                        _network,
                        ...
                        _location,
                        _channel,
                        _starttime.strftime("%Y.%j"),
                        _endtime.strftime("%Y.%j"),
                    )
                )
                try:
                    ...
                    if minradius:
                        err_msg = (
                            "No stations found for %s within radius %s-%s degrees of latitude,longitude %s,%s"
                            % (_sncl_pattern, minradius, maxradius, latitude, longitude)
                        )
                    else:
                ...
                self.logger.debug(
                    "Adding %s to the availability dataframe" % _sncl_pattern
                )

            else:
                # Read from FDSN web services
                self.logger.debug(
                    "read FDSN station web services %s for %s,%s,%s,%s,%s,%s"
                    % (
                        self.station_url,
                        _network,
                        _station,
                ...
                try:
                    sncl_inventory = self.station_client.get_stations(
                        starttime=_starttime,
                        endtime=_endtime,
                        network=_network,
                        station=_station,
                    ...
                    if re.match(
                        "The parameter 'includerestricted' is not supported by the service.",
                        str(e),
                    ):
                        try:
                            sncl_inventory = self.station_client.get_stations(
            ...
            py_pattern = (
                _sncl_pattern.replace(".", "\\.").replace("*", ".*").replace("?", ".")
            )
                ...
                for root, dirnames, fnames in os.walk(self.dataselect_url):
                    for fname in fnmatch.filter(fnames, fpattern1) + fnmatch.filter(
                        ...
                        if self.sds_files:
                            ...
                            if fnmatch.fnmatch(
                                position5, "[12][0-9][0-9][0-9]"
                            ) and fnmatch.fnmatch(position6, "[0-9][0-9][0-9]"):
                                file_year = int(position5)
                                file_day = int(position6)
                            else:
                                continue
                            ...
                            if fnmatch.fnmatch(
                                position4, "[12][0-9][0-9][0-9]"
                            ) and fnmatch.fnmatch(position5, "[0-9][0-9][0-9]"):
                                file_year = int(position4)
                                file_day = int(position5)
                            else:
                                continue
                ...
                for ii in range(len(df)):
                    ...
                    if lat and lon:
                        if not (math.isnan(lon) or math.isnan(lat)):
                            [dist, AB, BA] = obspy.geodetics.base.gps2dist_azimuth(
            ...
            if df.shape[0] == 0:
                self.logger.debug(
                    "No SNCLS found matching '%s' (sncl_format=%s)"
                    % (_sncl_pattern, self.sncl_format)
                )
            else:
        ...
        if len(sncl_pattern_dataframes) == 0:
            err_msg = "No available waveforms for %s matching " % _starttime.strftime(
                "%Y-%m-%d"
            ) + str(self.sncl_patterns)
            self.logger.info(err_msg)
            #             return pd.DataFrame()
            return None
            ...
            availability = pd.concat(
                sncl_pattern_dataframes, ignore_index=True, verify_integrity=True
            )
            ...
            availability = availability.drop("start", axis=1)
    ...
    def get_dataselect(
        ...
        if self.dataselect_type is None:
            ...
            nday = (
                int((_endtime - 0.00001).julday - _starttime.julday) + 1
            )  # subtract a short amount of time for 00:00:00 endtimes
                ...
                try:
                    ...
                    if len(matching_files) == 0:
                        ...
                        if len(matching_files) > 1:
                            self.logger.debug(
                                "Multiple files found: %s" % " ".join(matching_files)
                            )
                            self.logger.warning(
                                "Multiple files found matching "
                                "%s -- using %s" % (fpattern1, filepath)
                            )
                        ...
                        self.logger.debug(
                            "read local miniseed file for %s..." % filepath
                        )
                        py_stream = obspy.read(filepath).sort()

                        py_stream = py_stream.slice(
                            _starttime, _endtime, nearest_sample=False
                        )

                        if StrictVersion(obspy.__version__) < StrictVersion("1.1.0"):
                            flag_dict = obspy.io.mseed.util.get_timing_and_data_quality(
                                filepath
                            )
                            act_flags = [
                                0,
                    ...
                    if ignoreEpoch == False:
                        if len(availability) > 1:
                            raise Exception(
                                "Multiple metadata epochs found for %s" % _sncl_pattern
                            )
            ...
            try:
                self.logger.debug(
                    f"Dataselect found local data that spans {py_stream.traces[0].stats.starttime} - {py_stream.traces[-1].stats.endtime}"
                )
            except:
            ...
            try:
                ...
                r_stream = irisseismic.R_getDataselect(
                    self.dataselect_url,
                    self.dataselect_type,
                    network,
                    station,
                    location,
                    channel,
                ...
                if "No Data" in str(e):
                    self.logger.debug("No data available from %s" % self.dataselect_url)

                else:
                    err_msg = (
                        "Error reading in waveform from %s dataselect webservice client (base url: %s)"
                        % (self.dataselect_type, self.dataselect_url)
                    )
                    self.logger.error(err_msg)
                    self.logger.debug(str(e).strip("\n"))
                raise
            ...
            try:
                ...
                err_msg = "Error cutting R stream for start %s and end %s" % (
                    _starttime,
                    _endtime,
                )
                self.logger.debug(err_msg)
    ...
    def get_event(
        ...
        if self.event_client is None:
            ...
            for event in event_catalog:
                ...
                df = pd.DataFrame(
                    {
                        "eventId": re.sub(".*eventid=", "", event.resource_id.id),
                        "time": origin.time,
                        "latitude": origin.latitude,
                        "longitude": origin.longitude,
                        "depth": origin.depth
                        / 1000,  # QuakeML convention is meters, convert to kilometers
