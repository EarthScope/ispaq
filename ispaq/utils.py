...
def retrieve_psd_unique_targets(dbname, sncl_pattern, starttime, endtime, logger):
    ...
    conn = sqlite3.connect(dbname)
    select_sql = (
        "SELECT DISTINCT target from psd_corrected WHERE target like '"
        + sncl_pattern
        + "'"
    )
    if not starttime == "":
...
def write_simple_df(df, filepath, concierge, sigfigs=6):
    ...
    pretty_df = pretty_df.rename(
        index=str, columns={"snclq": "target", "starttime": "start", "endtime": "end"}
    )
...
def format_simple_df(df, sigfigs=6):
    ...
    * Convert 'starttime' and 'endtime' to python 'date' objects.
    ...
    if "value" in df.columns:
        ...
        df.loc[
            df["metricName"].str.match("timing_quality") & df["value"].str.match("nan"),
            "value",
        ] = "NULL"
...
def getSpectra(st, sampling_rate, metric, concierge):
    ...
    if respDir:
        ...
        localFile = os.path.join(
            respDir, ".".join(["RESP", network, station, location, channel])
        )  # attempt to find the RESP file
        localFile2 = os.path.join(
            respDir, ".".join(["RESP", station, network, location, channel])
        )  # alternate pattern
        for localFiles in (
            localFile,
            localFile + ".txt",
        ...
        if evalResp is None:
            raise EvalrespException(
                "No RESP file found at %s[.txt] or %s[.txt]" % (localFile, localFile2)
            )
        ...
        try:
            if concierge.station_type == None:
                ...
                evalResp = irisseismic.getEvalresp(
                    network=network,
                    station=station,
                    location=location,
                    channel=channel,
                ...
                concierge.logger.debug(
                    f"calling {concierge.station_url} - {concierge.station_type} evalresp web service"
                )
                evalResp = irisseismic.getEvalresp(
                    client_url=concierge.station_url,
                    client_type=concierge.station_type,
                    network=network,
                    station=station,
                    location=location,
                    channel=channel,
...
def getSampleRateSpectra(r_stream, sampling_rate, norm_freq, concierge):
    ...
    concierge.logger.debug(
        "minfreq %f, maxfreq %f, nfreq %f" % (minfreq, maxfreq, nfreq)
    )
    ...
    if respDir:
        ...
        try:
            evalResp = irisseismic.getEvalresp(
                concierge.dataselect_url,
                concierge.dataselect_type,
                network,
                station,
                location,
                channel,
