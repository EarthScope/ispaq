...
def orientationCheck_metrics(concierge):
    ...
    if concierge.station_client is None:
        try:
            initialAvailability = concierge.get_availability(
                "orientation", starttime=start, endtime=end
            )
    ...
    for index, event in events.iterrows():
        ...
        logger.info(
            "Magnitude %3.1f event: %s %sT%s:%s:%sZ"
            % (
                event.magnitude,
                event.eventLocationName,
                event.time.date,
                str(event.time.hour).zfill(2),
                str(event.time.minute).zfill(2),
                str(event.time.second).zfill(2),
            )
        )
        ...
        logger.debug(
            "Looking for metadata from %s to %s"
            % (
                halfHourStart.strftime("%Y-%m-%dT%H:%M:%S"),
                halfHourEnd.strftime("%Y-%m-%dT%H:%M:%S"),
            )
        )
        ...
        try:
            ...
            logger.warning(
                "Skipping event because concierge.get_availability failed: %s" % (e)
            )
            concierge.sncl_patterns = original_sncl_patterns
            continue
        ...
        for idx, sn_lId in enumerate(sorted(list(set(sn_lIds)))):
            ...
            logger.info(
                "%03d Calculating orientationCheck metrics for %s" % (idx, sn_lId)
            )
            ...
            if sn_lAvailability.shape[0] != 3:
                logger.info(
                    "Skipping %s because only %d channels were found at this SN.L (3 required)"
                    % (sn_lId, sn_lAvailability.shape[0])
                )
                continue
            ...
            if any(sn_lAvailability.samplerate.isnull().values):
                logger.info(
                    "Skipping %s because at least one channel is missing metadata"
                    % sn_lId
                )
                continue

            # Determine N, E and Z channels
            N_or_1_mask = sn_lAvailability.channel.str.contains(
                "..N"
            ) | sn_lAvailability.channel.str.contains("..1")
            E_or_2_mask = sn_lAvailability.channel.str.contains(
                "..E"
            ) | sn_lAvailability.channel.str.contains("..2")
            ...
            logger.debug(
                "Looking for data for %s, %s, %s from %s to %s"
                % (
                    Channel_1.snclId,
                    Channel_2.snclId,
                    ZChannel.snclId,
                    windowStart.strftime("%Y-%m-%dT%H:%M:%S"),
                    windowEnd.strftime("%Y-%m-%dT%H:%M:%S"),
                )
            )

            try:
                stN = concierge.get_dataselect(
                    Channel_1.network,
                    Channel_1.station,
                    Channel_1.location,
                    Channel_1.channel,
                ...
                if str(e).lower().find("no data") > -1:
                    ...
                    logger.warning(
                        ...
                        % (Channel_1.snclId, concierge.dataselect_url, e)
                    )
                continue

            try:
                stE = concierge.get_dataselect(
                    Channel_2.network,
                    Channel_2.station,
                    Channel_2.location,
                    Channel_2.channel,
                ...
                if str(e).lower().find("no data") > -1:
                    ...
                    logger.info(
                        "No data available for %s from %s: %s"
                        % (Channel_2.snclId, concierge.dataselect_url, e)
                    )
                continue

            try:
                stZ = concierge.get_dataselect(
                    ZChannel.network,
                    ZChannel.station,
                    ZChannel.location,
                    ZChannel.channel,
                ...
                if str(e).lower().find("no data") > -1:
                    ...
                    logger.info(
                        "No data available for %s from %s: %s"
                        % (ZChannel.snclId, concierge.dataselect_url, e)
                    )
                continue
            ...
            if (
                len(utils.get_slot(stN, "traces")) > 1
                or len(utils.get_slot(stE, "traces")) > 1
                or len(utils.get_slot(stZ, "traces")) > 1
            ):
            ...
            if abs(l1 - l2) > 1 or abs(l1 - l3) > 1:
                logger.info(
                    "Skipping %s because the number of data samples differs between channels. Incompatible lengths stN=%d, stE=%d, stZ=%d"
                    % (sn_lId, l1, l2, l3)
                )
                continue
            else:
            ...
            (stN, stE, stZ, HZ) = irisseismic.trim_taper_filter(
                stN, stE, stZ, max_length, taper, filterArgs
            )
            ...
            for angle in range(1, 360, degreeIncrement):
                ...
                try:
                    ...
                    logger.warning(
                        "skipping %s: irisseismic.rotate2D failed:  %s"
                        % (sn_lId, e.message)
                    )
                    rotateOK = False
                    break
            ...
            # Create metric
            df = irisseismic.generalValueMetric(
                utils.get_slot(stZ, "id"),
                windowStart,
                windowEnd,
                "orientation_check",
                elementNames,
