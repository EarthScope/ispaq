...
def calculate_PDF(fileDF, sncl, starttime, endtime, concierge):
    ...
    if concierge.output == "csv":
        ...
        psd = pd.concat(
            (
                pd.read_csv(snclFile, parse_dates=["starttime", "endtime"])
                for snclFile in snclFiles
            )
        )
        ...
        psd = psd[
            (psd["starttime"] >= starttime) & (psd["starttime"] < endtime)
        ]  # Changed to future-proof as PSDs will span the day-boundary
    ...
    index = pd.MultiIndex(levels=[[], []], codes=[[], []], names=["frequency", "power"])
    pdfDF = pd.DataFrame(
        columns=["frequency", "power", "hits"], index=index, dtype="object"
    )
    ...
    if pdfDF.empty:
        logger.info(
            "No PSDs found for %s, %s to %s"
            % (sncl, str(starttime).split("T")[0], str(endtime).split("T")[0])
        )
    ...
    for frequency in pdfDF["frequency"].unique():
        ...
        pdfDF.loc[pdfDF["frequency"] == frequency, "total"] = sum(
            pdfDF[pdfDF["frequency"] == frequency]["hits"]
        )
...
def plot_PDF(sncl, starttime, endtime, pdfDF, modesDF, maxsDF, minsDF, concierge):
    ...
    p1 = int(min(pdfDF["power"].unique()))
    p2 = int(max(pdfDF["power"].unique()))
    if p1 > -190:
        p1 = -190
    if p2 < -90:
        p2 = -90
    ...
    for power in powers:
        for freq in freqs:
            value = pdfDF[(pdfDF["frequency"] == freq) & (pdfDF["power"] == power)][
                "percent"
            ].values
            try:
    ...
    xlabels = [
        int(1 / i) if i <= 1 else 1 / i for i in xlabels
    ]  # convert to period, use decimal only if <1s
    ...
    plt.figure(figsize=(12, (0.055 * height + 0.5)))

    # Plot it up
    plt.imshow(
        plotList, cmap=cmap, vmin=0, vmax=30, aspect=0.4, interpolation="bilinear"
    )
