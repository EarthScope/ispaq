...
def crossCorrelation_metrics(concierge):
    ...
    if concierge.station_client is None:
        try:
            initialAvailability = concierge.get_availability(
                "polarityCheck", starttime=start, endtime=end
            )
    ...
    for index, event in events.iterrows():
        ...
        logger.debug(
            "Looking for metadata from %s to %s" % (halfHourStart, halfHourEnd)
        )
        try:
            availability = concierge.get_availability(
                ...
                minradius=eventMinradius,
                maxradius=eventMaxradius,
            )
        except NoAvailableDataError as e:
            ...
            logger.warning(
                "Skipping event %s %s  because concierge.get_availability failed: %s"
                % (event.magnitude, event.eventLocationName, e)
            )
            continue
        ...
        for index, av1 in availability.iterrows():
            ...
            dist = obspy.geodetics.base.locations2degrees(
                event.latitude, event.longitude, av1.latitude, av1.longitude
            )
            arrivals = model.get_travel_times(
                source_depth_in_km=event.depth, distance_in_degree=dist
            )
            ...
            logger.debug(
                "Looking for data for %s from %s to %s"
                % (av1.snclId, windowStart, windowEnd)
            )

            try:
                r_stream1 = concierge.get_dataselect(
                    av1.network,
                    av1.station,
                    av1.location,
                    av1.channel,
            ...
            logger.debug(
                "Looking for metadata for %s to %s within radius %s-%s degrees"
                % (halfHourStart, halfHourEnd, snclMinradius, snclMaxradius)
            )

            # Get the data availability using spatial search parameters
            try:
                availability2 = concierge.get_availability(
                    "polarityCheck",
                    network="*",
                    station="*",
                    location="*",
                    ...
                    starttime=halfHourStart,
                    endtime=halfHourEnd,
                    longitude=av1.longitude,
                    latitude=av1.latitude,
                    minradius=snclMinradius,
                    maxradius=snclMaxradius,
                )
            except Exception as e:
                logger.warning(
                    "Skipping %s because get_availability failed for nearby stations: %s"
                    % (av1.snclId, e)
                )
                continue
            ...
            if av1.channel[2] == "Z":
                ...
                azimuthAngle = (
                    abs(av1.azimuth - availability2.azimuth) * math.pi / 180.0
                )
            ...
            if not any(mask):
                ...
                for i in range(0, avCompatible.shape[0]):
                    ...
                    dist2.at[i] = obspy.geodetics.base.locations2degrees(
                        av1.latitude,
                        av1.longitude,
                        avCompatible.latitude.iloc[i],
                        avCompatible.longitude.iloc[i],
                    )
            ...
            for index2, av2 in avCompatible.iterrows():
                ...
                try:
                    ...
                    logger.warning(
                        "Skipping %s:%s because getTravelTime failed: %s"
                ...
                logger.debug(
                    "Looking for near neighbor station %s from %s to %s"
                    % (av2.snclId, windowStart, windowEnd)
                )

                try:
                    r_stream2 = concierge.get_dataselect(
                        av2.network,
                        av2.station,
                        av2.location,
                        av2.channel,
                    ...
                    if str(e).lower().find("no data") > -1:
                        ...
                        logger.info(
                            "Skipping %s because multiple metadata epochs are found"
                            % (av2.snclId)
                        )
                    else:
                        logger.warning(
                ...
                if sampler1 >= 1 and sampler2 >= 1:
                    ...
                    if (sr1 % sr2 != 0) and (sr2 % sr1 != 0):
                        logger.debug(
                            "Skipping %s:%s because actual sample rates are not compatible, %s:%s"
                            % (av1.snclId, av2.snclId, sr1, sr2)
                        )
            ...
            if testx == 1:
                logger.info(
                    "Skipping %s because no compatible stations found" % (av1.snclId)
                )
                continue
            ...
            if not r_stream2 == None:
                logger.info(
                    "%03d Calculating polarityCheck metrics for %s:%s"
                    % (index, av1.snclId, av2.snclId)
                )
                try:
                    df = irismustangmetrics.apply_correlation_metric(
                        r_stream1, r_stream2, "crossCorrelation", maxLagSecs
                    )
                    dataframes.append(df)

                except Exception as e:
                    logger.warning(
                        '"polarityCheck" metric calculation failed for %s:%s: %s'
                        % (av1.snclId, av2.snclId, e)
                    )
